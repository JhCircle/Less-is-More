...
def extract_train_role(strategy="with_few_shot"):
    with open(f"data/{strategy}_filtered.jsonl", "r", encoding="utf-8") as file:
    ...
    with open(f"data/train/{strategy}/training_question_parsing_role.jsonl", "w", encoding="utf-8") as output_file:
        for entry in question_parsing_data:
            output_file.write(json.dumps(entry, ensure_ascii=False) + "\n")

    with open(f"data/train/{strategy}/training_cot_verify_role.jsonl", "w", encoding="utf-8") as output_file:
        for entry in cot_verify_data:
            output_file.write(json.dumps(entry, ensure_ascii=False) + "\n")
        for entry in cot_statement_data:
            output_file.write(json.dumps(entry, ensure_ascii=False) + "\n")

    with open(f"data/train/{strategy}/training_cot_parsing_role.jsonl", "w", encoding="utf-8") as output_file:
        for entry in cot_parsing_data:
            output_file.write(json.dumps(entry, ensure_ascii=False) + "\n")

extract_train_role(strategy="with_few_shot")
extract_train_role(strategy="without_few_shot_")
extract_train_role(strategy="average")
